...
case class Organisation(id: String, code: String, email: String, sharedSecret: String)
...
object Organisation extends IdGenerator {
  ...
  implicit val rootJsonFormat: RootJsonFormat[Organisation] =
    jsonFormat4(Organisation.apply)
  ...
  implicit val encoder: Encoder[Organisation] = (a: Organisation) => Json.obj(
    ...
    "org_shared_secret" -> Json.fromString(a.sharedSecret))
  ...
  implicit val organizationDecoder: Decoder[Organisation] = (c: HCursor) => for {
    ...
    secret <- c.downField("org_shared_secret").as[String]
  } yield Organisation(uniqueId(), code, email, secret)
}